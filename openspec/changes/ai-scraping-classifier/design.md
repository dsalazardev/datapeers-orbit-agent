...
La llamada incluye los ≤10 candidatos (`title`, `url`, `source`) en un único `user` message con índices 1..N y exige una respuesta JSON estructurada como un array de `ProjectCard` alineado a esos índices. Razones: (1) minimiza latencia y coste (1 round-trip), (2) el límite natural de 10 ítems hace innecesaria la segmentación en múltiples llamadas, y (3) simplifica el fallback (todo o nada). La petición usa `response_format: {"type": "json_object"}`, cuyo soporte fue **verificado en vivo** (2026-09-17) sobre `deepseek/deepseek-v4-flash-0731:free` con una llamada mínima: devolvió `finish_reason=stop` y JSON válido (`{"ok": true}`) con coste 0; el modelo además anuncia `response_format` y `structured_outputs` en `supported_parameters`. Como los free-tier no garantizan retención del JSON, el prompt instructivo se mantiene y el parseo robusto (`strip` de ```` ```json ````) seguido de validación estricta contra `ProjectCard` siguen siendo obligatorios; si no valida → fallback completo. Además el parseo exige alineación 1:1 con la entrada: si `len(respuesta) != len(candidatos)` o los índices no alinean, la respuesta se trata como no válida → fallback completo.
